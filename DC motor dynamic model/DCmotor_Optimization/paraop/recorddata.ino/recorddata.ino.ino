...
void loop() {
  ...
  uint32_t period = 5000; // set the period to 5 seconds
  uint32_t tStart = millis(); // record the starting time
  while (millis() - tStart < period) { // keep looping for 5 seconds
    pwmOut(125);
    delay(0);
    noInterrupts();
    constCount = count;
  ...
  // print the position of the motor after 5 seconds
